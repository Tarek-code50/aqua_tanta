...
const MaintenanceForm: React.FC = () => {
  ...
  const inputClasses =
    'w-full p-3 rounded-lg border outline-none transition-colors bg-white text-slate-900 border-gray-300 placeholder:text-gray-400 focus:border-blue-500 focus:ring-2 focus:ring-blue-500 dark:bg-slate-800 dark:text-white dark:border-slate-700 dark:placeholder:text-gray-500';
    ...
    <div className="grid md:grid-cols-2 gap-8" dir={language === 'ar' ? 'rtl' : 'ltr'}>
      ...
      <div className="bg-white dark:bg-slate-800 p-8 rounded-2xl shadow-lg border-t-4 border-brand-500">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label
              className={`block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1 ${alignClass}`}
            >
      ...
      <div className="flex flex-col gap-8">
        <div className="bg-gradient-to-br from-slate-100 to-slate-50 dark:from-slate-800 dark:to-slate-900 p-8 rounded-2xl shadow-lg border border-slate-200 dark:border-slate-700">
          <div className="flex items-center gap-3 mb-4">
            ...
            <h3 className="text-xl font-bold text-slate-800 dark:text-white font-cairo">
              {t.maintenance.reminder}
            </h3>
          </div>

          <label
            className={`block text-sm font-medium text-slate-600 dark:text-slate-400 mb-2 ${alignClass}`}
          >
          ...
          <p className="text-xs text-slate-500 mt-2 mb-4">{t.maintenance.reminderBtn}</p>
          <button
