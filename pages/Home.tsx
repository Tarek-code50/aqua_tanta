    ...
    <div className="flex gap-16 animate-scroll w-[200%]">
      {[...Array(2)].map((_, i) => (
        <React.Fragment key={i}>
          <span className="text-3xl font-bold text-slate-300 dark:text-slate-600 grayscale hover:grayscale-0 transition-all">
            PANASONIC
          </span>
          <span className="text-3xl font-bold text-slate-300 dark:text-slate-600 grayscale hover:grayscale-0 transition-all">
            TANK
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 text-center">
          {stats.map((stat, idx) => (
            ...
            >
              ...
              <div className="text-3xl md:text-4xl font-extrabold mb-1 font-cairo drop-shadow-sm">
                {stat.value}
              </div>
              {stat.label && (
                <div className="text-blue-100 font-medium drop-shadow-sm">{stat.label}</div>
              )}
            </motion.div>
          ))}
        </div>
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-slate-50 dark:bg-slate-900">
        <div className="text-center p-10">
          ...
          <p className="text-slate-500">We&rsquo;re improving our waters. Be back soon.</p>
        </div>
      </div>
    );
  }
    ...
    <div id="home" className="overflow-x-hidden">
      ...
      <section className="relative min-h-[90vh] flex items-center overflow-hidden">
        ...
        <div
          ...
          style={{
            backgroundImage:
              'url("https://images.unsplash.com/photo-1548839140-29a749e1cf4d?q=80&w=2500&auto=format&fit=crop")',
          }}
        >
          {/* Gradient Overlay */}
          ...
          <div className="grid md:grid-cols-2 gap-12 items-center">
            ...
            >
              {features.map((f, i) => (
                <div
                  key={i}
                  className={`p-6 rounded-2xl bg-white/10 backdrop-blur-md border border-white/10 hover:bg-white/20 transition-all ${i === 1 ? 'col-span-2' : ''}`}
                >
      ...
      <section className="bg-slate-50 dark:bg-slate-900 py-24 relative">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-16 max-w-2xl mx-auto">
            <h2 className="text-3xl font-extrabold text-slate-900 dark:text-white mb-4 font-cairo">
              {t.comparison.title}
            </h2>
      ...
      <section className="py-24 bg-white dark:bg-slate-900">
        <div className="max-w-3xl mx-auto px-4">
          <div className="text-center mb-12">
            ...
            <h2 className="text-4xl font-extrabold text-slate-900 dark:text-white mt-2 font-cairo">
              {t.faq.title}
            </h2>
          </div>
